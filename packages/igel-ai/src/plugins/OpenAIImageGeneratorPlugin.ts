...
export class OpenAIImageGeneratorPlugin implements IImageGeneratorPlugin {
    ...
    async inpainting(
        ...
        if (
            !this._injectedMethods ||
            !this._injectedMethods.imageToFileObject
        ) {
        ...
        const imageFile = await this._injectedMethods.imageToFileObject(
            options.image
        );
        ...
        const response = await this._openai.createImageEdit(
            imageFile,
            options.mask
                ? await this._injectedMethods.imageToFileObject(options.mask)
                : imageFile,
            prompt,
    ...
    private generateImageRequest(
        ...
        if (options.width || options.height) {
            ...
            createImageRequest.size =
                `${options.width}x${options.width}` as CreateImageRequestSizeEnum;
        }
        if (options.responseType) {
            createImageRequest.response_format =
                options.responseType === "base64" ? "b64_json" : "url";
        }
        return createImageRequest;
    }

    private responseToReturnFormat(
        response: AxiosResponse<ImagesResponse, any>
    ) {
        return {
