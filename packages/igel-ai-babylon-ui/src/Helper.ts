...
export const resizeImage = async (imageUrl: string, maxDimension = 1200) => {
    const canvas = await imageUrlToCanvas(imageUrl);
    if(canvas.width <= maxDimension && canvas.height <= maxDimension) return imageUrl;
    const ctx = canvas.getContext('2d');
    if (!ctx) throw new Error('Could not get 2d context');
    const imageData = ctx.getImageData(0, 0, canvas.width, canvas.height);
    const data = imageData.data;
    ...
    newCtx.drawImage(canvas, 0, 0, newCanvas.width / ratio, newCanvas.height / ratio, 0, 0, newCanvas.width, newCanvas.height);
